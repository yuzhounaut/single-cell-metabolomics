...
#specify the features column names to be modelled, drop the first column
cell_model_df=cell_data_df.iloc[: , 1:]

from sklearn.ensemble import IsolationForest
CIF=IsolationForest(n_estimators=100, max_samples='auto', \
                        max_features=1.0, bootstrap=False, n_jobs=-1, random_state=42, verbose=0)
CIF.fit(cell_model_df)

pred = CIF.predict(cell_model_df)
...
shap_values = explainer.shap_values(cell_model_df)
shap_valuesh = explainer(cell_model_df)
...
shap.force_plot(explainer.expected_value, shap_values[0,:], cell_model_df.iloc[0,:], 
                plot_cmap="DrDb", matplotlib= True)
# Look at the 12th individual (which is an outlier) and the explanation of values
shap.force_plot(explainer.expected_value, shap_values[12,:], cell_model_df.iloc[12,:], 
                plot_cmap="DrDb", matplotlib= True)
# Look at the 97th individual and the explanation of values
shap.force_plot(explainer.expected_value, shap_values[97,:], cell_model_df.iloc[97,:], 
                plot_cmap="DrDb", matplotlib= True)
# Look at the 98th individual (which is an outlier)  and the explanation of values
shap.force_plot(explainer.expected_value, shap_values[98,:], cell_model_df.iloc[98,:], 
                plot_cmap="DrDb", matplotlib= True)

shap.plots.heatmap(shap_valuesh, max_display=10)

#The Decision Plot for Observation 1
shap.decision_plot(explainer.expected_value, shap_values[0,:], cell_model_df.iloc[0,:])

shap.summary_plot(shap_values, cell_model_df)
shap.summary_plot(shap_values, cell_model_df, plot_type="bar")
# visualize the first 5 predictions explanations with a dark red dark blue color map.
shap.force_plot(explainer.expected_value, shap_values[0:5,:], cell_model_df.iloc[0:5,:],
